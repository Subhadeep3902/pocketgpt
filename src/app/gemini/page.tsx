...
const Gemini: React.FC = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-black via-[#1f1f1f] to-black flex flex-col items-center justify-between h-screen p-3 px-52">
          ...
          <div className="w-full h-full flex flex-col text-white overflow-hidden">
            <div className="flex-1 p-4 overflow-y-auto">
              {messages.map((message, index) => (
                <div
                  key={index}
                  className={`mb-4 ${message.type === "ai" ? "ml-auto" : ""}`}
                >
                  <div
                    className={`p-4 shadow-lg backdrop-blur-md ${
                      message.type === "ai"
                        ? "bg-[rgba(34,40,49,0.8)] text-gray-100 max-w-2xl w-auto mr-auto rounded-lg"
                        : "bg-[rgba(214,90,49,0.8)] text-white max-w-md w-auto ml-auto rounded-3xl"
                    }`}
                  >
                    {message.text}
                  </div>
      ...
      <div className="w-full flex flex-col items-start justify-center h-40 mt-5">
